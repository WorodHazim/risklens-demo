...
export default function Home() {
  ...
  // Keyboard Accessibility for Side Panel
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isActionPanelOpen) return;
      if (e.key === 'Escape') setIsActionPanelOpen(false);
      if (e.key === 'Enter' && actionExecutionState === 'idle') handleExecuteAction();
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isActionPanelOpen, actionExecutionState]);
  ...
  const handleExport = (format: string) => {
    const caseId = selectedId || "GLOBAL";
    console.log(`Exporting ${format} for ${caseId}`);
    // Simulated visual feedback
    const toast = document.createElement('div');
    toast.className = "fixed bottom-8 right-8 bg-[#0A0A0A] border border-[#D4B483] px-6 py-3 rounded-sm shadow-2xl z-[100] animate-in slide-in-from-bottom duration-500 flex items-center gap-4";
    toast.innerHTML = `
      <div class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
      <div class="flex flex-col">
        <span class="text-[9px] font-bold text-[#D4B483] uppercase tracking-widest">System Export</span>
        <span class="text-[10px] text-[#CCC] font-mono">${format.toUpperCase()}_AUDIT_RECORD_GENERATED</span>
      </div>
    `;
  ...
  return (
    <main className="min-h-screen bg-[#050505] font-sans text-[#E5E5E0] pb-12 selection:bg-[#D4B483] selection:text-black">
      {/* Top Navigation Bar */}
      <nav className="bg-[#0A0A0A]/90 backdrop-blur-xl px-8 py-3.5 border-b border-[#1F1F1F] sticky top-0 z-50">
                  ...
                  <div className="bg-[#0A0A0A] rounded-sm border border-[#1F1F1F] p-0 overflow-hidden relative shadow-lg">
                      ...
                      <div className="flex flex-col md:flex-row gap-12 items-start">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-2xl text-[#E5E5E0] leading-relaxed font-light mb-12 border-l-[4px] border-[#D4B483]/30 pl-10 italic">
                            "{result?.explanation || "Awaiting secondary signal verification for final risk determination."}"
                          </p>
                          ...
                          <div className="grid grid-cols-2 gap-12 text-xs border-t border-[#1F1F1F] pt-10">
                            ...
                            <div className="flex items-center gap-6">
                              ...
                              <div>
                                ...
                                <span className="text-[#D4B483] font-mono font-medium text-lg">{result ? Math.round(result.confidence_score * 100) : 0}%</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <span className={`text-[10px] font-medium px-3 py-1.5 rounded-sm border transition-all flex items-center gap-2 ${isImpactExpanded ? 'bg-[#D4B483]/10 border-[#D4B483]/40 text-[#D4B483]' : 'bg-[#161616] border-[#262626] text-[#CCC] group-hover:border-[#333]'
                            ...
                            {result?.recommendation_impact || "N/A"}
                    ...
                    >
                      {getActionPanelData && (
                        <div className={`bg-[#0A0A0A] border rounded-sm p-6 shadow-inner relative overflow-hidden transition-all duration-700 ${impactGlow ? 'border-[#D4B483] shadow-[0_0_30px_rgba(212,180,131,0.15)] ring-1 ring-[#D4B483]/30' : 'border-[#1A1A1A]'}`}>
                          {/* Impact Section Header Row */}
                          <div className="flex justify-between items-center mb-5 pb-4 border-b border-[#1F1F1F]">
                            <div className="flex items-center gap-2.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-[#D4B483] animate-pulse"></span>
                              <span className="text-[9px] font-bold text-[#D4B483] uppercase tracking-[0.2em]">Scenario: No Intervention</span>
                            </div>
                            <div className="px-3 py-1.5 bg-[#D4B483]/5 border border-[#D4B483]/10 rounded-sm">
                              <span className="text-[8px] font-bold text-[#D4B483]/60 uppercase tracking-widest">Simulated Projection</span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                            ...
                            <div className="bg-[#0C0C0C] border border-[#1F1F1F] p-5 rounded-sm group hover:border-[#D4B483]/30 transition-colors relative" title="Projected outcome based on historical baselines.">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <span className="text-xs font-mono text-emerald-400 font-bold">{getActionPanelData?.impactMetrics.falsePositiveRate || "0%"}</span>
                              </div>
                    ...
                    {isActionPanelOpen && getActionPanelData && (
                      <div className="fixed inset-0 z-50 flex justify-end">
                        ...
                        {/* Panel */}
                        <div className={`relative w-full max-w-lg bg-[#0A0A0A] border-l ${result.risk_level === 'High' ? 'border-red-500/50' : 'border-[#1F1F1F]'} shadow-2xl animate-in slide-in-from-right duration-300 overflow-y-auto flex flex-col`}>
                          ...
                          <div className="sticky top-0 bg-[#0A0A0A] border-b border-[#1F1F1F] p-6 z-10">
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <h2 className="text-lg font-light text-[#E5E5E0]">{result?.recommended_action || "Pending Analysis"}</h2>
                              </div>
                              <button
                                onClick={() => setIsActionPanelOpen(false)}
                              ...
                              <div className="space-y-2">
                                {getActionPanelData.triggeredPolicies.map((policy, i) => (
                                  <div
                                    ...
                                    className="flex items-center justify-between bg-[#080808] border border-[#151515] rounded-sm px-4 py-3 cursor-help hover:border-[#D4B483]/30 transition-all group relative"
                                  >
                                    <div className="flex items-center gap-3">
                                    ...
                                    <svg className="w-3 h-3 text-[#333] group-hover:text-[#D4B483]/50 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>

                                    {hoveredPolicy === i && (
                                      <div className="absolute bottom-full left-0 mb-2 w-64 bg-[#111] border border-[#222] p-3 rounded-sm shadow-xl z-50 animate-in fade-in slide-in-from-bottom-1">
                                        <p className="text-[9px] text-[#D4B483] font-bold uppercase tracking-widest mb-1.5">Policy Details</p>
                                        <p className="text-[9px] text-[#777] leading-relaxed italic">AI detected pattern matching signature for {policy.split(':')[1].trim()}. Review threshold exceeded by {(Math.random() * 15 + 10).toFixed(1)}%.</p>
                                      </div>
                                    )}
                                  </div>
                                ))}
                          ...
                          <div className="sticky bottom-0 bg-[#0A0A0A] border-t border-[#1F1F1F] p-6">
                            <div className="mb-3 flex items-center justify-center gap-2">
                              ...
                              <p className="text-[9px] text-[#555] font-bold uppercase tracking-widest italic">
                                This action will create a permanent audit record.
                              </p>
                            </div>
                            ...
                            <button
                              ...
                              className={`w-full py-4 rounded-sm text-[11px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-3 active:scale-[0.98] ${actionExecutionState === 'idle'
                            ...
                            {/* Shortcut hint */}
                            <div className="mt-6 flex justify-between items-center px-4 py-2 border-t border-[#121212]">
                              <span className="text-[8px] text-[#333] font-mono tracking-tighter">SID: {selectedId?.slice(0, 8) || 'AUD-X82'}</span>
                              <span className="text-[8px] text-[#444] font-mono">Press <span className="text-[#666] border border-[#222] px-1 rounded-sm">ESC</span> to close • <span className="text-[#666] border border-[#222] px-1 rounded-sm">ENTER</span> to execute</span>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2 bg-[#0A0A0A] rounded-sm border border-[#1F1F1F] p-8 shadow-sm">
                      ...
                      <p className="text-sm font-light text-[#BBB] leading-loose">
                        {result?.business_impact || "Awaiting impact quantification..."}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-[#111] rounded-sm border border-[#1F1F1F] p-8 relative overflow-hidden">
                      ...
                      <p className="text-xs leading-loose text-[#888] font-mono pl-2 border-l border-[#333]">
                        &quot;{result?.why_not_low || "Primary signal deviance exceeds established low-risk entropy thresholds."}&quot;
                      </p>
                    </div>
                  </div>
