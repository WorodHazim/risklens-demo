...
export default function Home() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("risk_lens_storage");
    if (savedData) {
      const parsed = JSON.parse(savedData);
      setAuditLog(parsed.auditLog || []);
      setSavedNotes(parsed.savedNotes || {});
      // We could also persist updated scenarios here if we wanted strictly persistent simulation state
    }
  }, []);

  // Persistence: Save on Change
  useEffect(() => {
    localStorage.setItem("risk_lens_storage", JSON.stringify({
      auditLog,
      savedNotes
    }));
  }, [auditLog, savedNotes]);
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      setDecision(null); // Reset decision state for new view
      setSimulationAction("");
    }
  ...
  const analyzeRisk = useCallback(async (id: string, scenarioData?: any) => {
    setLoading(true);
    // Don't clear selectedId here to prevent UI flicker
  ...
  const handleExportQueue = () => {
    ...
    const rows = activeScenarios.map(s => [
      ...
      s.date,
      s.risk,
      // We don't have signals pre-computed for list view, so mock or leave empty
      "View Detail"
    ]);
