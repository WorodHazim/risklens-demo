...
export default function Home() {
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
             <div className="flex justify-between items-start mb-4">
                 <div>
                    <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Risk Distribution</h2>
             ...
             <div className="space-y-3 pt-4 border-t border-slate-100">
                <input 
                    ...
                    className="w-full text-xs border-slate-300 rounded px-2 py-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                />
                <div className="flex gap-2">
                    <select 
                        className="flex-1 text-xs border-slate-300 rounded px-2 py-1.5"
                        value={riskFilter}
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[500px]">
             <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                 ...
                 <span className="text-xs text-slate-500 bg-slate-200 px-2 py-0.5 rounded-full">{filteredScenarios.length}</span>
             </div>
  ...
  s.risk === 'High' ? 'bg-red-50 text-red-700 border-red-100' :
    s.risk === 'Medium' ? 'bg-amber-50 text-amber-700 border-amber-100' :
      'bg-emerald-50 text-emerald-700 border-emerald-100'
                 ...
                 {filteredScenarios.length === 0 && (
                     <div className="p-8 text-center text-xs text-slate-500 italic">No cases match your filters.</div>
                 )}
             </div>
          </div>

          {/* Decision Audit Trail */}
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 h-[250px] flex flex-col">
               <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">My Recent Activity</h2>
                    ...
                    {auditLog.length > 0 ? auditLog.map((log) => (
                        <div key={log.id} className="text-xs border-l-2 border-slate-200 pl-2 py-1">
                            <div className="flex justify-between text-slate-400 text-[10px]">
                                ...
                                <span>#{log.caseId?.substring(0,6).toUpperCase()}</span>
                            </div>
                            <p className="font-medium text-slate-700">
                                {log.outcome === "Accepted" ? "Recommendation Accepted" : "Decision Overridden"}
                            </p>
                        </div>
                    )) : (
                        <p className="text-xs text-slate-400 italic mt-4 text-center">No decisions logged this session.</p>
                    )}
                </div>
          </div>
          ...
          {result && analyzedScenario && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6 relative overflow-hidden">
               ...
               <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5 grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div>
                       <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Reasoning Analysis</h3>
                       <p className="text-sm text-slate-700 leading-relaxed">{getRiskExplanation(result.risk_level)}</p>
                   </div>
                   <div>
                       <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Why not lower?</h3>
                       <p className="text-sm text-slate-700 leading-relaxed">{getWhyNotLower(result.risk_level)}</p>
                   </div>
               </div>
                 ...
                 <div className="bg-slate-50 rounded-lg border border-slate-200 p-5">
                  ...
                  <div className="bg-purple-50 rounded-lg border border-purple-100 p-5">
                      ...
                      {simulationAction ? (
                          ...
                          <p className="text-xs text-purple-400 italic">Select an action to see predictive outcome.</p>
                      )}
                  </div>
              </div>
