...
export default function DashboardClient() {
    ...
    return (
        <main className="min-h-screen bg-background font-sans text-text-primary pb-12 selection:bg-cyan/20 selection:text-text-primary">
            <Header
                ...
                {currentView === "dashboard" && (
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        ...
                        <div className="lg:col-span-4 flex flex-col gap-10">
                            ...
                            <div className="bg-panel-bg/40 rounded-sm border border-border-main p-5 hover:border-cyan/30 transition-all cursor-pointer group panel-depth glow-cyan/5" onClick={() => setCurrentView('audit')}>
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-sm bg-background border border-border-main flex items-center justify-center group-hover:border-cyan/50 transition-colors">
                                            <span className="w-1.5 h-1.5 rounded-full bg-cyan/50 group-hover:bg-cyan animate-pulse"></span>
                                        </div>
                                        <div>
                                            <h2 className="text-[10px] font-bold text-text-primary uppercase tracking-[0.15em] mb-0.5 group-hover:text-cyan transition-colors">Ledger Persistence</h2>
                                            <p className="text-[9px] text-text-secondary font-mono tracking-tight">{auditLog.length} persistent block{auditLog.length !== 1 ? 's' : ''} locked</p>
                                        </div>
                                    </div>
                                    <svg className="w-3 h-3 text-text-secondary/40 group-hover:text-cyan transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5l7 7-7 7" /></svg>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-8 space-y-12">
                            {!result ? (
                                <div className="h-full min-h-[600px] flex flex-col items-center justify-center bg-panel-bg/20 rounded-sm border border-border-main border-dashed text-text-secondary/20 technical-border animate-fade-in">
                                    <div className="w-20 h-20 mb-6 opacity-10 relative animate-sentinel">
                                        <Image src="/risklens-logo.svg" alt="Logo" fill className="object-contain" />
                                    </div>
                                    <div className="text-center max-w-sm px-8">
                                        <p className="text-[10px] font-bold text-text-primary mb-3 uppercase tracking-[0.25em]">Sentinel HUD Operational</p>
                                        <p className="text-[9px] text-text-secondary uppercase tracking-widest leading-loose opacity-60">Select a neural vector from the priority queue to initialize adaptive risk reasoning and governance chain</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500 ease-out">
                                    {/* Analysis Header */}
                                    <div className="bg-panel-bg rounded-sm border border-border-main p-0 overflow-hidden relative shadow-2xl technical-border glow-gold/5">
                                        <div className={`h-1 w-full ${result.risk_level === "High" ? "bg-gold" : result.risk_level === "Medium" ? "bg-gold/60" : "bg-cyan/60"}`}></div>
                                        <div className="p-10">
                                            <div className="flex flex-col md:flex-row gap-12 items-start">
                                                <div className="flex-shrink-0 flex flex-col items-center">
                                                    <span className="text-9xl font-thin tracking-tighter text-text-primary drop-shadow-[0_0_20px_rgba(255,255,255,0.05)]">{result.risk_score}</span>
                                                    <span className={`mt-4 px-6 py-2 rounded-sm text-[11px] font-bold uppercase tracking-[0.3em] border shadow-2xl transition-all ${getRiskBadgeColor(result.risk_level)}`}>
                                                        {result.risk_level} Risk
                                                    </span>
                                                </div>
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-8">
                                                        <h2 className="text-[11px] font-bold text-text-secondary uppercase tracking-[0.3em] flex items-center gap-2">
                                                            <span className="w-1 h-1 bg-cyan rounded-full animate-pulse"></span>
                                                            Decision Reasoning Engine
                                                        </h2>
                                                        <span className="text-[9px] font-mono text-text-secondary px-3 py-1 bg-background border border-border-main tracking-tighter uppercase">ID: {selectedId?.toUpperCase()}</span>
                                                    </div>
                                                    <p className="text-2xl text-text-primary leading-relaxed font-light mb-12 border-l-[3px] border-gold/40 pl-10 italic">
                                                        "{result.explanation}"
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="bg-background/40 px-10 py-6 border-t border-border-main flex justify-between items-center scan-overlay">
                                            <div className="flex items-center gap-6">
                                                <span className="text-[9px] font-bold text-text-secondary uppercase tracking-[0.2em] opacity-60">Recommended Mitigation:</span>
                                                <button
                                                    onClick={() => setIsActionPanelOpen(true)}
                                                    className="px-6 py-2.5 bg-background border border-border-main text-gold text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-gold hover:text-background hover:border-gold transition-all flex items-center gap-3 active:scale-[0.98] glow-gold/20"
                                                >
            ...
            {/* Global Toast */}
            {toast && (
                <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 px-6 py-4 rounded-sm border shadow-2xl z-[200] animate-in fade-in slide-in-from-bottom-4 duration-300 flex items-center gap-4 ${toast.type === 'success' ? 'bg-emerald/10 border-emerald/50 text-emerald' : 'bg-panel-bg/90 border-gold/50 text-gold shadow-gold/20'
                    }`}>
                    <div className={`w-2 h-2 rounded-full ${toast.type === 'success' ? 'bg-emerald animate-pulse' : 'bg-gold animate-pulse'}`}></div>
