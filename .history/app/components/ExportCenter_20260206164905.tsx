...
export default function ExportCenter({ activeScenarios, auditLog }: ExportCenterProps) {
    ...
    const handleDownload = (type: string, format: string, data: any) => {
        let content = "";
        let mimeType = "";
        const fileName = `${type}_export_${new Date().toISOString().split('T')[0]}.${format.toLowerCase()}`;
        ...
        if (format === "JSON") {
            ...
            content = `<?xml version="1.0" encoding="UTF-8"?><export><type>${type}</type><data>${JSON.stringify(data)}</data></export>`;
            mimeType = "application/xml";
        } else if (format === "PDF") {
            // Mock PDF content since we don't have a PDF lib
            content = `RISKLENS AUDIT DOCUMENT\nTYPE: ${type.toUpperCase()}\nDATE: ${new Date().toISOString()}\n\nRECORDS: ${JSON.stringify(data, null, 2)}`;
