...
export default function DashboardClient() {
    ...
    const handleExecuteAction = (outcome: string = "Approved", notes: string = "") => {
        ...
        setTimeout(() => {
            const newRecord: AuditRecord = {
                ...
                caseId: selectedId,
                outcome: outcome,
                notes: notes || `AI Recommendation [${result.recommended_action}] executed by ${userRole}.`
            };

            setAuditLog(prev => [newRecord, ...prev]);
            setCaseStatus(prev => ({ ...prev, [selectedId]: 'RESOLVED' }));
            setActionExecutionState('applied');
            setToast({ message: `Action [${outcome}] successfully executed and archived.`, type: 'success' });
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <div className="lg:col-span-4 space-y-8">
                            <PriorityQueue
                                ...
                                onSelect={(id) => {
                                    analyzeRisk(id);
                                    if (isActionPanelOpen) setIsActionPanelOpen(false);
                                }}
                        ...
                        <div className="lg:col-span-8 space-y-12">
                            {!result ? (
                                ...
                                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-700">
                                    {/* Analysis Header */}
                                    <div className="bg-[#0A0A0A] rounded-sm border border-[#1F1F1F] p-0 overflow-hidden relative shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
                                        ...
                                        <div className="p-10">
                                            <div className="flex flex-col md:flex-row gap-12 items-start">
                                                <div className="flex-shrink-0 flex flex-col items-center">
                                                    <span className="text-9xl font-thin tracking-tighter text-[#F5F5F0] drop-shadow-2xl">{result.risk_score}</span>
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-8">
                                                        <h2 className="text-[11px] font-bold text-[#888] uppercase tracking-[0.3em]">AI Reasoning Chain</h2>
                                                        <span className="text-[10px] font-mono text-[#D4B483]/70 px-3 py-1 bg-[#111] border border-[#222]">CASE: {selectedId?.toUpperCase()}</span>
                                                    </div>
                                                    <p className="text-2xl text-[#F5F5F0] leading-relaxed font-light mb-12 border-l-[4px] border-[#D4B483]/30 pl-10 italic">
                                                        "{result.explanation}"
                                                    </p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="bg-[#0C0C0C] px-10 py-6 border-t border-[#1F1F1F] flex justify-between items-center">
                                            <div className="flex items-center gap-4">
                                                <span className="text-[10px] font-bold text-[#444] uppercase tracking-widest">Recommended Action:</span>
                                                <div className="px-4 py-2 bg-[#161616] border border-[#222] text-[#D4B483] text-[11px] font-bold uppercase tracking-widest flex items-center gap-2">
                                                    {result.recommended_action}
                                                    <div className="w-1.5 h-1.5 rounded-full bg-[#D4B483]/50 animate-pulse"></div>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] font-mono text-[#333] uppercase">Confidence: {Math.round(result.confidence_score * 100)}%</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {/* Evaluation Slot: Ensures layout height is stabilized during transition */}
                                    <div className="min-h-[200px] flex flex-col gap-12">
                                        <HumanReview
                                            result={result}
                                            selectedId={selectedId || ''}
