        ...
        <div className="max-w-[1500px] mx-auto flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6 px-6 border-l border-r border-[#1F1F1F]">
            <div
              ...
              title="Total number of cases pending analyst review."
            >
              <span className="text-[9px] font-bold text-[#555] uppercase tracking-wider">Active</span>
              <span className="text-sm font-bold text-[#E5E5E0] font-mono">{activeScenarios.length}</span>
            </div>
            <div
              className="flex items-center gap-2"
              title="Cases requiring immediate attention (SLA < 4 hours)."
            >
              <span className="text-[9px] font-bold text-[#555] uppercase tracking-wider">Urgent</span>
              <span className={`text-sm font-bold font-mono ${activeScenarios.filter(s => s.risk === 'High').length > 0 ? 'text-red-400' : 'text-[#666]'}`}>
                {activeScenarios.filter(s => s.risk === 'High').length}
              </span>
            </div>
            <div
              className="flex items-center gap-2"
              title="Decisions logged in current session."
            >
              <span className="text-[9px] font-bold text-[#555] uppercase tracking-wider">Resolved</span>
              <span className="text-sm font-bold text-emerald-500/80 font-mono">{auditLog.length}</span>
            </div>
          </div>
              ...
              <div className="relative" id="user-profile-dropdown">
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 top-11 w-48 bg-[#0F0F0F] border border-[#222] rounded-sm shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-150">
                    ...
                    <div className="p-1">
                      <button className="w-full text-left px-3 py-2 text-[10px] uppercase font-bold text-red-500/80 hover:text-red-400 hover:bg-[#1A1A1A] rounded-sm transition-colors" onClick={() => console.log('Sign Out')}>
                        Sign Out
                      </button>
                    </div>
                  </div>
                ...
                {auditLog.length > 0 ? (
                  <table className="w-full text-left text-xs">
                    <thead className="bg-[#080808] text-[#444] font-semibold border-b border-[#1A1A1A]">
                      <tr>
                        <th className="px-6 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Unique hash fingerprint for tamper detection">HASH ID</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="UTC timestamp of record creation">TIMESTAMP</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Source of the action: AI recommendation or Human analyst">ACTOR</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Reference to the case file">CASE REF</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Type of decision recorded">ACTION</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Mandatory justification for audit trail">JUSTIFICATION</span>
                        </th>
                        <th className="px-4 py-4 font-bold text-[8px] uppercase tracking-wider text-[#333]">
                          <span title="Record immutability status">SEAL</span>
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#111]">
                      {auditLog.map((log, index) => {
                        const hash = `0x${log.id.slice(0, 4)}...${log.id.slice(-4)}`;
                        const isOverride = log.outcome === 'Overridden';

                        return (
                          <tr key={log.id} className={`hover:bg-[#0C0C0C] transition-colors ${isOverride ? 'bg-amber-950/5' : ''}`}>
                            {/* Hash ID */}
                            <td className="px-6 py-4 font-mono text-[9px] text-[#444]" title={`Full hash: ${log.id}`}>
                            ...
                            {/* Timestamp */}
                            <td className="px-4 py-4 font-mono text-[9px] text-[#555] whitespace-nowrap">
                            ...
                            <td className="px-4 py-4">
                              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-sm text-[8px] font-bold uppercase tracking-wider ${isOverride
                                ? 'text-cyan-400/80 bg-cyan-950/20 border border-cyan-900/30'
                                : 'text-violet-400/80 bg-violet-950/20 border border-violet-900/30'
                                }`}>
                                <span className={`w-1 h-1 rounded-full ${isOverride ? 'bg-cyan-400' : 'bg-violet-400'}`}></span>
                                {isOverride ? 'HUMAN' : 'AI + HUMAN'}
                              </span>
                            </td>

                            {/* Case Reference */}
                            <td className="px-4 py-4 font-mono text-[10px] text-[#D4B483]">
                              #{log.caseId?.substring(0, 8).toUpperCase()}
                            </td>

                            {/* Action Type */}
                            <td className="px-4 py-4">
                              <span className={`inline-flex items-center px-2.5 py-1 rounded-sm text-[9px] font-bold uppercase tracking-tight ${isOverride
                                ? 'text-amber-400/90 bg-amber-950/20 border border-amber-900/40'
                                : 'text-emerald-400/90 bg-emerald-950/20 border border-emerald-900/40'
                                }`}>
                            ...
                            {/* Justification */}
                            <td className="px-4 py-4 text-[10px] text-[#666] max-w-xs">
                              <div className="flex items-start gap-2">
                                <span className="text-[8px] text-[#333] font-mono shrink-0">NOTE:</span>
                                <span className="truncate font-mono" title={log.notes}>{log.notes || '—'}</span>
                              </div>
                            </td>

                            {/* Seal Status */}
                            <td className="px-4 py-4">
                              <span className="flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-widest text-emerald-600/60" title="Record is cryptographically sealed and immutable">
                    ...
                    filteredScenarios.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-4 rounded-sm border transition-all duration-200 group relative ${isResolved
                        ...
                        >
                          <div className="flex justify-between items-start mb-2.5">
                            <div className="flex items-center gap-2">
                              ...
                              {/* Status Badge */}
                              <span className={`flex items-center gap-1 text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${status === 'NEW' ? 'text-cyan-400/80 bg-cyan-950/30 border border-cyan-900/40' :
                                status === 'IN-REVIEW' ? 'text-amber-400/80 bg-amber-950/30 border border-amber-900/40' :
                                  'text-emerald-400/80 bg-emerald-950/30 border border-emerald-900/40'
                                }`}>
                                <span className={`w-1 h-1 rounded-full ${status === 'NEW' ? 'bg-cyan-400' :
                                  status === 'IN-REVIEW' ? 'bg-amber-400 animate-pulse' :
                                    'bg-emerald-400'
                                  }`}></span>
                                {status}
                              </span>
                            ...
                            <div className="flex items-center gap-2">
                              {isAging && (
                                <span className="text-[8px] font-bold text-amber-500/70 uppercase tracking-wider animate-pulse">AGING</span>
                              )}
                              <span className={`text-[9px] font-bold uppercase tracking-wide ${sla.color}`}>{sla.text}</span>
                            </div>
                          </div>

                          <h3 className={`text-sm font-medium mb-1 truncate pr-4 ${isResolved ? "text-[#555]" : isSelected ? "text-[#E5E5E0]" : "text-[#777] group-hover:text-[#AAA]"}`}>
                            {s.name}
                          </h3>
                          <p className={`text-[9px] font-mono uppercase tracking-tight mb-4 ${isResolved ? "text-[#333]" : isSelected ? "text-[#D4B483]" : "text-[#333] group-hover:text-[#444]"}`}>
                            // TRIGGER: {context}
                          </p>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {openedAt && status !== 'NEW' && (
                                <span className="text-[8px] text-[#444] font-mono">
                                  {agingMinutes < 60 ? `${agingMinutes}m in queue` : `${Math.floor(agingMinutes / 60)}h ${agingMinutes % 60}m`}
                                </span>
                              )}
                            </div>
                    ...
                    <div className="bg-[#0C0C0C] px-10 py-6 border-t border-[#1F1F1F] flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <button
                          ...
                          className={`text-xs font-bold uppercase tracking-wide flex items-center gap-3 px-4 py-2 rounded-sm shadow-sm group cursor-pointer transition-all ${actionExecutionState === 'applied'
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <span className="text-[10px] font-medium text-[#CCC] bg-[#161616] px-3 py-1.5 rounded-sm border border-[#262626] group-hover:border-[#333] transition-colors flex items-center gap-2">
                            {result.recommendation_impact}
                            <svg className={`w-3 h-3 text-[#555] transition-transform ${isImpactExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                            </svg>
                          </span>
                        </button>

                        {/* Impact Expansion Panel */}
                        {isImpactExpanded && getActionPanelData && (
                          <div className="absolute top-full right-0 mt-2 w-80 bg-[#0A0A0A] border border-[#1F1F1F] rounded-sm shadow-xl z-20 animate-in fade-in slide-in-from-top-2 duration-200">
                            <div className="p-4 border-b border-[#151515]">
                              <h4 className="text-[9px] font-bold text-[#666] uppercase tracking-widest mb-3">Impact Metrics</h4>
                              <div className="space-y-3">
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] text-[#777]">False Positive Rate</span>
                                  <span className="text-[10px] font-mono text-emerald-400">{getActionPanelData.impactMetrics.falsePositiveRate}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] text-[#777]">SLA Improvement</span>
                                  <span className="text-[10px] font-mono text-[#D4B483]">{getActionPanelData.impactMetrics.slaImprovement}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] text-[#777]">Analyst Workload</span>
                                  <span className="text-[10px] font-mono text-cyan-400">{getActionPanelData.impactMetrics.analystWorkload}</span>
                                </div>
                              </div>
                            </div>
                            <div className="p-3 text-center">
                              <span className="text-[8px] text-[#444] uppercase tracking-wider">Based on similar historical cases</span>
                            </div>
                          </div>
                        )}
                      </div>
